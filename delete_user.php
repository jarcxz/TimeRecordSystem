...
?>
	...
	$admin_name = $f_adminname['username'];
			...
			$user=$conn->query("insert into backup_employee  select * from tbl_employee where company_number = '$id' ");
			$user=$conn->query("insert into backup_login_logs  select * from check_in where company_number = '$id' ");
	
			$user = $conn->query("delete from tbl_employee where company_number = '$id' ") or die(mysqli_error());
